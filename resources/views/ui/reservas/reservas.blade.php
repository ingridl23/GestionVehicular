...
{{-- DIALOGS — siempre en el DOM, fuera del @if isEmpty --}}
...
<dialog id="dialog-cancelar" class="p-0 backdrop:bg-black/50 rounded-lg">
  <div class="bg-gray-800 rounded-lg shadow-xl w-full max-w-md mx-auto">
    <div class="p-6">
      <div class="flex items-center gap-4">
        <div class="flex h-12 w-12 items-center justify-center rounded-full bg-red-500/10">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="h-6 w-6 text-red-400">
    ...
    <div class="flex justify-end gap-3 bg-gray-700/30 px-6 py-4 rounded-b-lg">
      ...
      <button type="button" class="botonCancelar px-4 py-2 text-sm font-medium text-white bg-red-500 hover:bg-red-400 rounded-md">
        Desactivar
      </button>
    </div>
  </div>
  ...
  <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-md mx-auto">
    <div class="p-6">
      <div class="flex items-center gap-4">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Aprobar reserva</h3>
      </div>
      <p class="mt-4 text-sm text-gray-600 dark:text-gray-300">
        ¿Está seguro de que desea aprobar esta reserva?
        El vehículo quedará asignado y pasará a estado <strong>EN USO</strong>.
      </p>
    </div>
    <div class="flex justify-end gap-3 bg-gray-100 dark:bg-gray-800 px-6 py-4 rounded-b-lg">
      <button type="button" onclick="this.closest('dialog').close()"
      ...
      <button type="button" class="botonAprobarReserva px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md">
        Aprobar reserva
      </button>
    </div>
  </div>
...
<script>
  window.RESERVAS_CONFIG = {
    permissions: {
      ...
      aprobar: @json(auth()->user()->can('autorizar_reservas_internas')),
      mostrarAcciones: "{{ $mostrarAcciones }}",
    },
    routes: {
      ver: "{{ route('reservas.reserva', ':id') }}",
      editar: "{{ $configEditar['route'] }}",
      aprobar: "{{ route('admin.reservas.aprobar', ':id') }}",
    }
  };
